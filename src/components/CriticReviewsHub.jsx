...
export default function CriticReviewsHub({ reviews, onNavigate, onSelectCritic }) {
  ...
  const filteredReviews = useMemo(() => {
    ...
    let list = reviews.filter(r => {
      const oakRemark = getOakShowRemark(r.score != null ? r.score : r.remark);
    ...
    return list.sort((a, b) => {
      ...
      if (sortBy === 'title-desc') return (b.targetTitle || b.title || '').localeCompare(b.targetTitle || b.title || '');
  ...
  return (
    <div className="container critic-hub-root animate-fade-in">
      {/* Header Banner */}
      <div className="catalog-header-banner glass-panel">
        <div className="badge badge-gold">
          <Award size={14} />
          VERIFIED CRITIC OPINIONS
        </div>
        <h2 className="catalog-main-title">OakShow Editorial & Critic Reviews</h2>
        <p className="catalog-subtitle">
          Unbiased, in-depth film criticism, legacy ratings, and certified recommendations by OakShow reviewers.
        </p>
      </div>

      <div className="critic-hub-actions-bar glass-panel" style={{ padding: '16px 24px', marginBottom: '24px', borderRadius: '16px' }}>
      ...
      {activeTab === 'all-reviews' && (
        <div className="reviews-feed-view animate-fade-in">
          ...
          <div className="glass-panel" style={{ marginBottom: '24px', padding: '24px 32px', borderRadius: '20px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {/* Search Input */}
            <div className="critic-search-wrap" style={{ margin: 0 }}>
                    ...
                    <div className="review-content-body">
                      <div className="review-rating-score-row">
                        <div className="rating-pill">
                          ...
                          <span>{rev.score != null ? `${rev.score}/5` : rev.rating}</span>
                        </div>
                        {rev.date && (
        ...
        <div className="review-reader-backdrop animate-fade-in" onClick={() => setActiveReviewModal(null)}>
          <div className="review-reader-dialog" onClick={e => e.stopPropagation()}>
            ...
            <div className="reader-body">
              {(() => {
                const modalRemark = getOakShowRemark(activeReviewModal.score != null ? activeReviewModal.score : activeReviewModal.remark);
                return (
                  <>
                    <div className="reader-meta-bar">
                      <div className="rating-pill">
                        <img src={modalRemark.icon} alt={modalRemark.title} className="oakshow-cert-icon-inline" />
                        <span>Rating: {activeReviewModal.score != null ? `${activeReviewModal.score}/5` : activeReviewModal.rating}</span>
                      </div>
      ...
      <style>{`
        ...
        .critic-profile-card {
          ...
          padding: 20px;
          border-radius: 16px;
          background: var(--bg-surface, rgba(18, 24, 38, 0.7));
        ...
        .critic-sort-dropdown {
          background: var(--bg-surface, rgba(10, 14, 26, 0.85));
        ...
        .review-card {
          ...
          border-radius: 20px;
          overflow: hidden;
          background: var(--bg-surface, rgba(18, 24, 38, 0.6));
        ...
        .review-card:hover {
          ...
          border-color: var(--accent-primary, rgba(255, 255, 255, 0.15));
          box-shadow: 0 12px 24px rgba(0, 0, 0, 0.15), 0 0 16px var(--accent-primary-glow, rgba(245, 158, 11, 0.08));
          background: var(--bg-surface-elevated, rgba(18, 24, 38, 0.85));
        }
        .review-banner-wrap {
          position: relative;
          width: 100%;
        ...
        .review-card:hover .read-review-btn {
          color: var(--accent-primary-hover, #fbbf24);
          gap: 12px;
        }
